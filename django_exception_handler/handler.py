...
from .persian_error_codes import persianEnglishErrorCodesDict
...
def _get_farsi_detail(exception_code, attr, persianDict):
    if exception_code in persianEnglishErrorCodesDict:
        error_dict = persianEnglishErrorCodesDict.get(exception_code)
        if error_dict.get('has_attribute') is True and attr is not None:
            attrs = str(attr).split('.')[-1]
            return error_dict.get('farsi_detail_attrs').replace('attrs', persianDict.get(attrs, attrs))
            # return error_dict.get('farsi_detail_attrs').replace('attrs', _(attrs))
        else:
            return error_dict.get('farsi_detail')
    return None
